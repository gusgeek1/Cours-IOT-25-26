...
# Dossier dans lequel se trouve le script
...
# Dictionnaire utilisé pour mémoriser la dernière valeur de batterie reçue
last_known_battery = {}
...
def ensure_nichoir_exists(nichoir_id):
    try:
        ...
        # Création du nichoir s’il n’est pas présent en base
        if not result:
            sql = """
...
# Si des données image sont fournies, elles sont sauvegardées sur le disque
def save_capture(nichoir_id, photo_data=None):
    try:
        ...
        # Insertion de la capture dans la base de données
        conn = get_db()
        cursor = conn.cursor()
...
# Fonction appelée automatiquement à chaque réception d’un message MQTT
def on_message(client, userdata, msg):
    try:
        ...
        if msg_type == 'batterie':
            ...
            try:
                valeur_batt = float(payload)
                last_known_battery[nichoir_id] = valeur_batt
            except ValueError:
                print("Erreur de conversion de la valeur batterie")
