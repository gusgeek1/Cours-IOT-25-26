...
# Mot de passe administrateur pour l’accès au dashboard
...
def estimate_autonomy(v):
    # Si aucune valeur n’est disponible, une valeur neutre est retournée
    if not v:
        return "--"
    ...
    # Estimation de l’autonomie sur base de 6 mois pour une batterie pleine
    mois = int((pct / 100) * 6)
...
def index():
    ...
    if not session.get('logged_in'):
        if request.method == 'POST':
            # Vérification du mot de passe envoyé via le formulaire
