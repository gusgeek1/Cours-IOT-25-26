...
# Récupération du dossier dans lequel se trouve le script
...
def ensure_nichoir_exists(nichoir_id):
    try:
        ...
        # Insertion du nichoir s’il n’est pas encore présent
        if not result:
            sql = """
...
# Enregistre une photo et les informations associées dans la table Captures
def save_capture(nichoir_id, photo_data=None):
    try:
        ...
        # Si des données image sont reçues, elles sont sauvegardées sur le disque
        if photo_data:
        ...
        # Conversion en entier pour correspondre au type attendu en base de données
...
# Fonction appelée automatiquement lorsqu’un message MQTT est reçu
def on_message(client, userdata, msg):
    try:
        ...
        if msg_type == 'status':
            ...
            # Stockage de la valeur reçue sous forme de float
...
# Association de la fonction de réception des messages
