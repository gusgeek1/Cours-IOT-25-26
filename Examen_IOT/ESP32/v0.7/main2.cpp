...
// ------------------------------------------------
//      ⭐ /capture — PREND UNE PHOTO
// ------------------------------------------------
void handleCapture() {

  Serial.println("Capture photo…");

  lastPhoto = TimerCAM.Camera.fb;  // clear previous frame (safety)

  if (!TimerCAM.Camera.get()) {
    server.send(500, "text/plain", "Erreur capture photo");
    return;
  }

  lastPhoto = TimerCAM.Camera.fb;  // keep pointer

  // Redirect browser to the JPEG file
  server.sendHeader("Location", "/photo.jpg", true);
  server.send(302, "text/plain", "");
}

// ------------------------------------------------
//      ⭐ /photo.jpg — ENVOIE LA PHOTO
// ------------------------------------------------
void handlePhoto() {
  if (!lastPhoto) {
    server.send(404, "text/plain", "Aucune photo");
    return;
  }
...
// ------------------------------------------------
//      PAGE CONFIGURATION (v2 + bouton capture v4)
// ------------------------------------------------
void handleRoot() {
...
<!-- ⭐ BOUTON TEST CAMERA (v4) -->
<button formaction="/capture" formmethod="POST">Tester la caméra</button>

<button type="submit">Enregistrer</button>
</form>

<div class="footer">Projet IOT - 2025-2026 - Collard T. & Dejonghe A.</div>
<div class="footer">v0.4</div>
